...
# Get the rgb calibration for all colors
...
# Determines if the sensor is over a specific color based off the rgb values and the range
...
# Turns the robot right using the inner barrier
...
# Turns the robot left using the outer barrier
...
# Simulates the robot getting bumped from behind and speeds the robot up for a second
def gotBumped():
    base = motor_pair.get_default_speed()
